...
function CoverUpload(props: CoverUploadProps) {
  const { existingImageUrl = null, name } = props;
  const inputId = useId();

  const { displayImageUrl, fileInputRef, handleFileChange } = useImagePreview({
    existingImageUrl,
  });

  return (
    <CoverImagePlaceholder
      className={cn('p-8', displayImageUrl && 'border-transparent')}
    >
      {displayImageUrl && (
        <Image
          src={displayImageUrl}
